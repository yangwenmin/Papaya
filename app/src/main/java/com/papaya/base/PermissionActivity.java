...
public class PermissionActivity extends SupportActivity {
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        switch (requestCode) {
            case ConstValues.HARDWEAR_CAMERA_CODE:
                ...
                break;
            case ConstValues.WRITE_READ_EXTERNAL_CODE:
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    doWriteSDCard();
                } else {
            ...
            case ConstValues.LOCAL_CODE:
                ...
                break;
            case ConstValues.WRITE_LOCAL_CODE:
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    doCameraWriteSD();
                } else {
                    Toast.makeText(getApplicationContext(), "请先开启读写存储权限", Toast.LENGTH_SHORT).show();
                ...
                break;
            case ConstValues.LOCATION_ALL_CODE:
                ...
                break;
            case ConstValues.LOCATION_WEB_CODE:
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    doWebLocation();
                } else {
                    Toast.makeText(getApplicationContext(), "请先开启定位权限", Toast.LENGTH_SHORT).show();
                ...
                break;
            case ConstValues.READ_CALL_LOG_CODE:
                ...
                break;
            case ConstValues.CALL_PHONE_CODE:
